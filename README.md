...
- **歌词显示（逐字卡拉OK模式）**：点击展开后，歌词区以卡拉OK方式显示——**当前句的字逐个点亮**——高亮进度为连续值（非整数跳字），边界以「线性渐变 + 60fps 缓动」持续平滑扫过，不停顿；当前句高亮仅由卡拉OK文字渐变体现（无背景胶囊，避免双重高亮；20px 大号），其余句淡化，**平滑滚动自动居中**（60fps 逐帧逼近当前句，展开即自动跟随）；紧凑态左对齐实时显示当前句并同样逐字点亮；可选独立悬浮歌词小窗。
